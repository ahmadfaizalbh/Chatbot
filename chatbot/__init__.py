...
class Chat(object):
    ...
    def __block_tags(self, text, pos, length, index):
        """
         text: (str) template file
         pos: (list) list of tuples
                    (tag_content_start, tag_content_pos_end, tag_name, '')
         length:(int) total count of pos list
         index:(int) current processing tag index inside the pos list

         Process the each block tags then
         return the index and content inside the block.
    ...
    def __group_tags(self, text, pos, groups, condition, length, index=0, name=""):
        ...
        pos: (list) list of tuples
                    (tag_content_start, tag_content_pos_end, tag_name, '')
        groups: (dict) groups key values pairs
        condition: (function) condition to process the tag
        length:(int) total count of pos list
        index:(int) current processing tag index inside the pos list

        Iterate the processed template based on block type (block, response, group).
        Group tags are recursively processed. Both are append to pairs and defaults.
        """
        pairs = []
        defaults = []
    ...
    def __process_template_file(self, file_name):
        """
         ...
         return: (dict) Each group response separate by group name.

         Read the template file and generate the list of tuple
         (tag_start_pos, tag_end_pos, tag_name, '') for each block tags.

         Then, Update groups dictionary with specific group. If group name is not given
         then it'll consider as general group.
        """
